...
def _extract_parts(parts: list) -> list[dict]:
    ...
    for p in parts:
        ...
        if isinstance(root, TextPart) and getattr(root, "text", None):
            ...
            # If wrapped as {"kind": "data", "metadata": {"mimeType": ...}, "data": {...}}
            if isinstance(raw_data, dict) and "data" in raw_data:
                ...
                if isinstance(inner_meta, dict) and inner_meta.get("mimeType") == _A2UI_MIME:
                    ...
                    continue
                elif any(k in raw_data["data"] for k in ("beginRendering", "surfaceUpdate", "dataModelUpdate", "deleteSurface")):
                    out.append({"kind": "a2ui", "data": raw_data["data"]})
                    continue
            if mime == _A2UI_MIME or (isinstance(raw_data, dict) and any(k in raw_data for k in ("beginRendering", "surfaceUpdate", "dataModelUpdate", "deleteSurface"))):
